...
class SimulatorConfig(object):
    ...
    def __init__(
        ...
        assert (
            len(agent_color) == self.color_num_dims
        ), "Agent color must have the same dimension as item colors"
        assert all(
            [len(i.scent) == self.scent_num_dims for i in items]
        ), "All items must use the same dimensionality for the scent vector."
        assert all(
            [len(i.color) == self.color_num_dims for i in items]
        ), "All items must use the same dimensionality for the color vector."
        assert all(
            [len(i.required_item_counts) == len(items) for i in items]
        ), "The `required_item_counts` field must be the same dimension as `items`"
        assert all(
            [len(i.required_item_costs) == len(items) for i in items]
        ), "The `required_item_costs` field must be the same dimension as `items`"
        assert all(
            [len(i.interaction_fns) == len(items) for i in items]
        ), "The `interaction_fn_args` field must be the same dimension as `items`"
...
class Simulator(object):
    ...
    def __init__(
        ...
        conn_queue_capacity=256,
        num_workers=8,
        on_lost_connection_callback=None,
        save_frequency=1000,
        save_filepath=None,
        load_filepath=None,
        load_time=-1,
        ...
        if load_filepath != None and load_time < 0:
            raise ValueError(
                'If "load_filepath" is specified, "load_time" must also be specified as a non-negative integer.'
            )
        ...
        if sim_config != None:
            ...
            if load_filepath != None:
                raise ValueError(
                    '"load_filepath" must be None if "sim_config" is specified.'
                )
            elif server_address != None:
                raise ValueError(
                    '"server_address" must be None if "sim_config" is specified.'
                )
            self._handle = simulator_c.new(
                sim_config.seed,
                sim_config.max_steps_per_movement,
                [d.value for d in sim_config.allowed_movement_directions],
                [d.value for d in sim_config.allowed_turn_directions],
                sim_config.no_op_allowed,
                sim_config.scent_num_dims,
                sim_config.color_num_dims,
                sim_config.vision_range,
                sim_config.patch_size,
                sim_config.mcmc_num_iter,
                [
                    (
                        i.name,
                        i.scent,
                        i.color,
                        i.required_item_counts,
                        i.required_item_costs,
                        i.blocks_movement,
                        i.visual_occlusion,
                        i.intensity_fn,
                        i.intensity_fn_args,
                ...
                sim_config.agent_color,
                sim_config.collision_policy.value,
                sim_config.agent_field_of_view,
            ...
            if self._client_id == 0:
                (
                    ...
                    self._client_handle,
                    self._client_id,
                ) = simulator_c.connect_client(
                    server_address,
                    port,
                    self._step_callback,
                    on_lost_connection_callback,
                )
            else:
                ...
                for i in range(len(agent_ids)):
                    ...
                    (
                        ...
                        agent._items,
                        agent._id,
                    ) = (position, Direction(direction), scent, vision, items, id)
        else:
            # load local server or simulator from file
            if load_filepath == None:
                raise ValueError(
                    '"load_filepath" must be non-None if "sim_config" and "server_address" are None.'
                )
            self._load_agents(load_filepath, load_time)
            (self._time, self._handle, agent_states) = simulator_c.load(
                load_filepath + str(load_time), self._step_callback
            )
            for agent_state in agent_states:
                ...
                (
                    ...
                    agent._vision,
                    agent._items,
                ) = (position, Direction(direction), scent, vision, items)
            if is_server:
                self._server_handle = simulator_c.start_server(
                    self._handle,
                    port,
    ...
    def _add_agent(self, agent):
        ...
        Returns:
          The new agent's ID.
        """
        (position, direction, scent, vision, items, id) = simulator_c.add_agent(
            self._handle, self._client_handle
        )
    ...
    def move(self, agent, direction, num_steps=1):
        ...
        """
        return simulator_c.move(
            self._handle, self._client_handle, agent._id, direction.value, num_steps
        )
    ...
    def turn(self, agent, direction):
        ...
        Returns:
          `True`, if successful; `False`, otherwise.
        """
        return simulator_c.turn(
            self._handle, self._client_handle, agent._id, direction.value
        )
    ...
    def get_permissions(self, client_id):
        ...
        if self._server_handle == None:
            raise RuntimeError(
                "`get_permissions` requires that the Simulator be a server."
            )
        perm = Permissions()
        (
            perm.add_agent,
            ...
            perm.get_map,
            perm.get_agent_ids,
            perm.get_agent_states,
            perm.manage_semaphores,
            perm.get_semaphores,
        ) = simulator_c.get_permissions(self._server_handle, client_id)
        return perm
    ...
    def _map(self, bottom_left, top_right):
        ...
        """
        return simulator_c.map(
            self._handle, self._client_handle, bottom_left, top_right, True, False
        )
    ...
    def _save_agents(self):
        with open(self._save_filepath + str(self._time) + ".agent_info", "wb") as fout:
            ...
            for agent_id, agent in self.agents.items():
                agent.save(
                    self._save_filepath + str(self._time) + ".agent" + str(agent_id)
                )
